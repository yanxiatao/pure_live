...
上游基线：`liuchuancong/pure_live@99ef708c`
发布日期：2026-08-24

## 本轮范围

- 合并上游多画面同看、播放器结构整理和依赖锁更新，保持与上游共同祖先关系。
- 合并上游视频显示模式与房间增量合并，并补齐迟到请求围栏、稀疏状态保持、本地标签保护和播放器内核原生画面适配。
...
- 修复关注自定义标签选中态未订阅、无法切回“全部”，并分别约束搜索结果回弹与平台分类栏边界。
- 修复弹幕原生描边被四次偏移重复绘制；低透明度使用连续对比度曲线，改善亮色画面的边缘清晰度并减少重复绘制。
...
- #782：真实接口确认推荐列表混合回放；应用同时支持房间页与列表播放结构。最新反馈刷新失效 Cookie 后已恢复，剩余“全部失败”属于平台风控状态。
- #783：移除四层偏移描边叠加，并在低透明度下保留连续轮廓对比度。
- #784：修复“全部 → 自定义 → 全部”标签往返；搜索结果恢复触摸回弹，平台栏首尾不再被拖离。
